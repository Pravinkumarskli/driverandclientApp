...
/**
 * NativeSocketService — Unified Facade for clientapp coordinating ChatSocketService,
...
class NativeSocketService {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        if (!isSettled) {
          ...
          console.warn("[NativeSocketService] Socket registration timeout reached.");
          resolve(false);
        }
      }, timeoutMs);
  ...
  sendCustomerLocation(locationData) {
    return TrackingSocketService.sendCustomerLocation(locationData, this.currentUserId);
  }
