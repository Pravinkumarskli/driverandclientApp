 ...
 * All operations are fire-and-forget except load (awaited by caller).
 */
import AsyncStorage from "@react-native-async-storage/async-storage";

const KEY_PREFIX = "chat_messages_";
const MAX_STORED_MESSAGES = 200; // Keep last 200 per conversation
...
/**
 * Load all messages for a conversationId from local storage.
...
/**
 * Save a single new message into local storage for a conversation.
 * Merges with existing messages, deduplicates by messageId/id,
 * and trims to MAX_STORED_MESSAGES.
 */
export async function saveMessage(conversationId, message) {
  try {
    ...
    if (alreadyExists) {
      // Update status only if message already exists
...
/**
 * Update the status of an existing message (e.g. "sending" -> "delivered").
 */
...
/**
 * Clear all stored messages for a conversation.
 */
