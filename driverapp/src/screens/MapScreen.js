...
// Overview zoom level matching the initial view
  ...
  try {
    ...
    if (data.code === 'Ok' && data.routes && data.routes.length > 0) {
      ...
      if (coords && coords.length > 1) {
        console.log(
          '🛣️ [DRIVER OSRM ROAD ROUTE] Coordinates fetched:',
          coords.length,
          'points',
        );
        return coords;
...
export default function MapScreen({ route, navigation }) {
  ...
  // 1. FIXED Customer Home Location [longitude, latitude] (Stationary Destination)
  const [homeLocation, setHomeLocation] = useState([79.8083, 11.9416]);

  // 2. MOVING Driver Car Location [longitude, latitude] (Moves along road towards Home)
      ...
      try {
        const granted = await PermissionsAndroid.request(
          PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
          {
            ...
            message: 'Driver App needs your location for road navigation.',
            buttonPositive: 'OK',
          },
        );
  ...
  // Generate Road Route from Driver starting point (~3.5km away) to Fixed Customer Home
  const generateRoadRoute = useCallback(async (fixedHomeLngLat) => {
    try {
      ...
      console.log(
        '📍 [DRIVER MAP] Generating road route from Driver -> Customer Home:',
        driverStart,
        '->',
        fixedHomeLngLat,
      );
  ...
  // Fetch Home/Destination GPS ONCE on start & connect driver socket
  useEffect(() => {
    let isMounted = true;
          ...
          Geolocation.getCurrentPosition(
            (pos) => {
              if (isMounted && pos?.coords) {
                ...
                console.log('🏠 [FIXED CUSTOMER HOME GPS SET]:', fixedHome);
  ...
  // 10-Second Continuous CAR-ONLY Movement along Road Route & Live Socket Broadcast
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // When car reaches Home (end of route), loop back to the 3.5km start point
      if (nextIndex >= pts.length) {
        console.log(
          '🚗 [DRIVER CAR ARRIVED AT CUSTOMER HOME] -> Restarting from 3.5km start point',
        );
        nextIndex = 0;
      }
      ...
      console.log(
        `🚗 [DRIVER CAR MOVING] Step ${nextIndex + 1}/${pts.length}:`,
        nextCarPos,
      );
      setCarLocation(nextCarPos);

      // Broadcast Driver Live GPS to Customer via Socket
      SocketService.sendLocation({
        driverId,
        latitude: nextCarPos[1],
        longitude: nextCarPos[0],
        accuracy: 4,
        speed: 38 + (nextIndex % 6),
        heading: 90,
        timestamp: Date.now(),
      });
      ...
      <View style={styles.container}>
        ...
        <View style={styles.mapContainer}>
          <Map
            ...
            attribution={false}
          >
            {/* initialViewState sets starting zoom and center position */}
            <Camera
              ref={cameraRef}
              initialViewState={{
                center: [
            ...
            <GeoJSONSource id="driverRouteSource" data={routeGeoJSON}>
              <Layer
                id="driverRouteLine"
                type="line"
                paint={{
            ...
            {/* 🏠 STATIONARY CUSTOMER HOME MARKER (Fixed Destination) */}
            ...
            {/* 🚗 MOVING CAR MARKER (Moves every 10s along road & restarts from 3.5km away) */}
          ...
          <View style={styles.mapHeader} pointerEvents="box-none">
            ...
            <View style={styles.headerRight}>
              <TouchableOpacity
                ...
                onPress={() =>
                  Alert.alert(
                    'Driver Dispatch Helpline',
                    'Contacting 24/7 Driver Support: +91 1800 987 6543',
                  )
                }
                activeOpacity={0.8}
              >
        ...
        >
          ...
          <View style={styles.deliveryCard}>
            <View style={styles.deliveryInfo}>
              ...
              <Text style={styles.deliverySubtitle}>
                Passenger Pickup • 12 min Estimated
              </Text>
            </View>
          ...
          <View style={styles.driverCard}>
            ...
            <View style={styles.driverInfo}>
              ...
              <Text style={styles.driverRole}>Passenger</Text>
            </View>
            ...
            <View style={styles.reviewHeader}>
              ...
              <TouchableOpacity
                onPress={() =>
                  Alert.alert(
                    'Trip Details',
                    `Booking: ${bookingNumber}\nPassenger: ${customerName}\nStatus: Live GPS Dispatch Active\nFare: ₹350`,
                  )
                }
              >
          ...
          {/* MapButton for External Google Maps Navigation */}
