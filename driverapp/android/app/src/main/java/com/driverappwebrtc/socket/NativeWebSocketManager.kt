...
class NativeWebSocketManager(
    ...
    companion object {
        private const val TAG = "NativeWSManager_Driver"
    ...
    private var userType: String = "driver"
    ...
    fun stop() {
        ...
        try {
            webSocket?.close(1000, "Driver explicit disconnect")
        } catch (e: Exception) {
    ...
    private fun connect() {
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(ws: WebSocket, response: Response) {
                Log.d(TAG, "⚡ Native Driver WebSocket Opened with server. Registering userId: $userId ($userType)")
                isConnected = true
                reconnectAttempts = 0
    ...
    private fun handleIncomingMessage(rawJson: String) {
        try {
            ...
            if (eventType == "registerSuccess") {
                ...
                Log.d(TAG, "👤 Driver Registration confirmed by server for $registeredUid ($registeredType)")
                isRegistered = true
    ...
    private fun scheduleReconnect() {
        ...
        Log.d(TAG, "Scheduling driver reconnect attempt #$reconnectAttempts in ${delay}ms")
        mainHandler.removeCallbacksAndMessages(null)
    ...
    override fun onNetworkLost() {
        Log.w(TAG, "⚠️ Network lost. Marking driver socket as disconnected.")
        isConnected = false
        isRegistered = false
        listener.onConnectionStateChanged("DISCONNECTED")
    }
